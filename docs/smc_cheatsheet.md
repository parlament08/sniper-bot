...
| `Trigger` | почему 15m trigger не стал подтвержденным входом |
...
`Trigger Debug` отвечает на вопрос:

```text
Почему 15m trigger не стал настоящим входом?
```

Теперь trigger-кандидаты разделены по направлению:

| Поле | Что означает |
| --- | --- |
| `long_trigger_candidate` | последний bullish 15m BOS/CHoCH/MSS |
| `short_trigger_candidate` | последний bearish 15m BOS/CHoCH/MSS |
| `scenario_trigger_15m` | trigger, который прошел post-SFP / post-POI scanner |
| `trigger_scan` | подробный результат временной проверки trigger после anchor |
| `selected_trigger` | trigger нужного направления для текущего LONG/SHORT сценария |
| `candidate_trigger` | лучший видимый trigger-кандидат; в NEUTRAL это диагностика, а не вход |
| `opposite_trigger` | trigger противоположного направления, полезен для диагностики, но не для входа |
| `anchor_index` | индекс SFP или POI, после которого scanner ищет подтверждение |

Правило:

```text
LONG может выбрать только bullish trigger.
SHORT может выбрать только bearish trigger.
Trigger должен появиться после SFP или POI, иначе он сохраняется как pre_sfp/pre_poi,
но не используется для A+ сценария, state machine и risk confirmation.
Для NEUTRAL нет выбранного направления, поэтому trigger показывается как candidate,
а не как opposite.
```

Если бот смотрит LONG, но на 15m найден только `bearish_bos`,
это не `selected_trigger`, а `opposite_trigger`.

Пример:

```text
🔎 Trigger Scan: rejected — bullish BOS Q88 was before SFP
```

APE-like пример:

```text
🔎 Trigger Scan: waiting — no bullish CHOCH/BOS after SFP/POI | opposite: bearish BOS Q81
```

NEUTRAL пример:

```text
🔎 Trigger Scan: skipped — no trade direction because HTF is Neutral | candidate: bearish CHOCH Q84
```

### Scenario Scan

`Scenario Scan` отвечает на более широкий вопрос:

```text
Есть ли живой LONG/SHORT сценарий по времени, и на каком шаге он сейчас стоит?
```

Он не заменяет `Trigger Scan`. `Trigger Scan` проверяет только post-SFP / post-POI trigger,
а `Scenario Scan` собирает всю цепочку:

```text
HTF context -> POI -> SFP -> CHOCH -> BOS -> FVG -> retest -> displacement -> risk
```

Правила:

```text
CHOCH/BOS до SFP не подтверждают сценарий.
BOS до CHOCH в strict mode инвалидирует сценарий.
FVG до BOS инвалидирует сценарий.
Displacement до FVG retest инвалидирует сценарий.
Risk invalid учитывается только после того, как сценарий дошел до risk/entry этапа.
```

Примеры:

```text
🧭 Scenario Scan: waiting for bullish CHOCH after SFP | 3/10 steps
🧭 Scenario Scan: complete LONG scenario | 10/10 steps | A+ allowed
🧭 Scenario Scan: invalidated — fvg_before_bos
🧭 Scenario Scan: no valid scenario — HTF neutral
```

A+ gate:

```text
A+ разрешен только если Scenario Scan complete, scenario_valid=true,
signal_allowed=true и risk_plan.valid=true.
```

Если score уже >= 70, но сценарий неполный или сломан:

```text
score capped to 69
decision = Watchlist
no_trade_reason = scenario_scan.reason
```
...
| --- | --- |
| `no_15m_trigger_found` | 15m BOS/CHoCH не найден |
| `no_bullish_trigger_after_sfp_or_poi` | для LONG нет bullish trigger после SFP/POI |
| `no_bearish_trigger_after_sfp_or_poi` | для SHORT нет bearish trigger после SFP/POI |
| `trigger_direction_conflict` | trigger есть, но в другую сторону |
| `trigger_before_sfp` | trigger появился до sweep, порядок неправильный |
| `trigger_before_poi` | trigger появился до теста POI/FVG |
| `trigger_quality_below_min` | trigger слабее минимального качества |
| `trigger_without_poi_or_sfp` | trigger есть, но без связки с POI/SFP |
| `fvg_too_old` | FVG слишком старый для сценария |
| `fvg_quality_below_min` | FVG слишком слабый для сценария |
| `fvg_too_many_retests` | зону слишком много раз тестировали |
| `stale_event` | событие слишком далеко по времени от trigger |
| `accepted` | trigger прошел debug-фильтр |

Важно:

```text
Trigger Debug не добавляет баллы.
Он объясняет, почему Gate не пропустил сетап.
```
...
| `scenario_valid` | может ли FVG участвовать в сценарии State Machine |

FVG теперь имеет два смысла:

```text
информационный FVG = можно показать в отчете и оценить quality;
scenario FVG = достаточно свежий и качественный, чтобы быть шагом State Machine.
```

Для State Machine FVG должен пройти фильтр:
...
```text
FVG Q40 age203 retests8 -> scenario_valid=False
```

Такой FVG может остаться в отчете, но не должен ломать сценарий как `FVG_CREATED`.
