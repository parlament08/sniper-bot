...
def scan_scenarios(
    *,
    ...
    valid_candidates = [item for item in (best_long, best_short) if item and item.status != "invalidated"]
    if not valid_candidates:
        ...
        return ScenarioScannerOutput(
            ...
            signal_allowed=False,
            scenario_valid=False,
            reason=(expected_invalid.invalidated_reason if expected_invalid and expected_invalid.invalidated_reason else "no_valid_scenario"),
        )

    selected = max(valid_candidates, key=lambda item: _selection_rank(item, expected_direction, htf_trend))
    return ScenarioScannerOutput(
        best_long_scenario=best_long,
        best_short_scenario=best_short,
...
def _scan_direction(
    ...
    *,
    ...
    if htf_trend in ("bullish", "bearish") and htf_trend != scenario_side:
        return _invalid_result(direction, "htf_direction_conflict")

    if premium_discount is not None and not _pd_valid_for_direction(direction, premium_discount):
        return _invalid_result(direction, "pd_invalid_for_direction")
    ...
    for event in events:
        ...
        if event_type == "INVALIDATION" and _event_applies(direction, event):
            return _invalid_result(direction, _payload_reason(event, "invalidation"), used)
        ...
        if event_type == "HTF_CONTEXT_CONFIRMED":
            if side == "neutral":
                return _invalid_result(direction, "htf_direction_conflict", used)
        ...
        if event_type == "FVG_CREATED" and side == scenario_side:
            if _payload_bool(event, "invalidated"):
                last_invalidated_component = "fvg_invalidated"
                continue
            if not bos_seen:
        ...
        if event_type == "FVG_RETESTED" and side == scenario_side:
            if not fvg_seen:
                last_invalidated_component = "retest_before_fvg"
                continue
            if completed < 7:
                used.append(event)
        ...
        if event_type == "DISPLACEMENT_CONFIRMED" and side == scenario_side:
            if not retest_seen:
                last_invalidated_component = "displacement_before_retest"
                continue
            if completed < 8:
                used.append(event)
    ...
    if completed >= len(FLOW):
        return _complete_result(direction, used, risk_valid=risk_valid, risk_reason=risk_reason)
    return _partial_result(
        direction,
        "waiting_for_confirmation",
        used,
